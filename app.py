...
with st.expander("❓ 각 항목의 데이터는 어디서 찾나요? (클릭해서 펼쳐보기)"):
    st.markdown("""
    * **현재 잉여현금흐름(FCF):** [네이버 증권] > 종목검색 > 종목분석 > 기업재무분석 표 맨 아래 'FCF' 수치 입력 
      (미국주는 [야후 파이낸스] > Financials > Cash Flow > 'Free Cash Flow' 입력)
    * **총 발행 주식 수:** [네이버 증권] 종목 메인 화면 우측 '상장주식수' 전체 숫자 입력 
      (미국주는 [야후 파이낸스] > Statistics > 'Shares Outstanding' 확인)
    * **할인율(%):** 워렌 버핏의 기회비용입니다. 무난한 우량주는 **8.5%**, 미래가 다소 불확실한 위험 주식은 **10%**를 입력하세요.
    * **예상 성장률(%):** [네이버 증권] '컨센서스' 탭의 증권사 추정치 참고. (보통 안정적인 성숙기업 3-5%, 우량 기술주 10-15%, 초고속 성장주 20% 이상)
    """)

# 3. 시장 선택 (단위 자동 변환을 위한 핵심 로직!)
market_type = st.radio("🌍 주식 시장을 선택하세요", ("한국 주식 (FCF 단위: 억원)", "미국 주식 (FCF 단위: 백만 달러)"))

# 4. 구독자 입력 칸 만들기 (기본값 세팅)
current_fcf = st.number_input("1. 현재 잉여현금흐름 (FCF 숫자만 입력)", value=500.0)
shares = st.number_input("2. 총 발행 주식 수 (주)", value=60000000)
...
if st.button("적정 주가 구하기"):
    ...
    # 🎯 단위 환산 로직 적용
    if market_type == "한국 주식 (FCF 단위: 억원)":
        actual_fcf = current_fcf * 100000000  # 1억 곱하기
        currency = "원"
    else:
        actual_fcf = current_fcf * 1000000    # 100만 곱하기
        currency = "달러"
    ...
    # 결과 화면 출력
    st.success(f"💡 계산된 1주당 적정 주가는 **{int(price_per_share):,} {currency}** 입니다!")
